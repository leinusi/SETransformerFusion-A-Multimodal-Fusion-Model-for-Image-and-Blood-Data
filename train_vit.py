...
# 训练主干网络(特征提取器)
criterion = nn.CrossEntropyLoss() 
optimizer_backbone = optim.SGD(feature_extractor.parameters(), lr=0.001, momentum=0.9,weight_decay=0.01)
num_epochs_backbone = 10
...
# 定义优化器和损失函数
optimizer = optim.SGD(list(feature_extractor.parameters()) + list(se_transformer_fusion.parameters()), lr=0.001, momentum=0.9,weight_decay=0.01)
criterion = nn.CrossEntropyLoss()
